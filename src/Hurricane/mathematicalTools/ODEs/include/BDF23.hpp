/*!
 * \file BDF23.hpp
 * \brief Headers of BDF23 ODE solver.
 *        The subroutines and functions are in the <i>BDF23.cpp</i> file.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    /**
     * \brief The class of adaptive step BDF23 solver.
     */
    class BDF23 : public ODEsSolver {
    private:
        realArray dfdt_;
        realArray dydt_;
        ...
        integer iOrder_;
        integer iCorder_;
        void setAlpha(const integer iord);

        realArray lastTimeStep_;

        real initialFct_;

        integer countJacStep_;

    public:
        declareClassNames;

        BDF23(const integer nEqs);

        BDF23(const integer nEqs, const real atol, const real rtol, const integer maxStep = 1000);

        BDF23(const integer nEqs, const controller &cont);

        /**\brief Destructor.*/
        virtual ~BDF23() noexcept {}
