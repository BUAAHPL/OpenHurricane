 ...
 * \brief Headers of Implicit Euler ODE solver.
 *        The subroutines and functions are in the <i>ImplicitEuler.cpp</i> file.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    /**
     * \brief The class of adaptive step Implicit Euler solver.
     */
    class ImplicitEuler : public ODEsSolver {
    private:
        realArray dfdt_;
        realArray dydt_;
    ...
    public:
        ...
        ImplicitEuler(const integer nEqsNew);

        ImplicitEuler(const integer nEqsNew, const real atol, const real rtol,
                      const integer maxStep = 1000);

        ImplicitEuler(const integer nEqsNew, const controller &cont);

        /**\brief Destructor.*/
        virtual ~ImplicitEuler() noexcept {}
