/*!
 * \file hexRegion.cpp
 * \brief Main subroutines for hexahedron region of mesh.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    createClassNameStr(hexRegion, "hex");
}
namespace OpenHurricane {
    registerObjFty(markRegion, hexRegion, controller);
}

OpenHurricane::hexRegion::hexRegion(const controller &cont)
    : markRegion(cont), xmax_(cont.findOrDefault<real>("xmax", real(0.0))),
      xmin_(cont.findOrDefault<real>("xmin", real(0.0))),
      ymax_(cont.findOrDefault<real>("ymax", real(0.0))),
      ymin_(cont.findOrDefault<real>("ymin", real(0.0))),
      zmax_(cont.findOrDefault<real>("zmax", real(0.0))),
      zmin_(cont.findOrDefault<real>("zmin", real(0.0))) {}

hur_nodiscard OpenHurricane::integerList
OpenHurricane::hexRegion::regionCellId(const runtimeMesh &mesh) const {
    integerList cid;
...
bool OpenHurricane::hexRegion::patching(realGeometryArray<cellMesh> &cellQ, real &value) const {
    return patch(cellQ, value);
}

bool OpenHurricane::hexRegion::patching(vectorGeometryArray<cellMesh> &cellQ, vector &value) const {
    return patch(cellQ, value);
}

bool OpenHurricane::hexRegion::distributing(realGeometryArray<cellMesh> &cellQ,
...
bool OpenHurricane::hexRegion::distributing(vectorGeometryArray<cellMesh> &cellQ,
                                            std::string &value) const {
    const auto &mesh = cellQ.mesh();
    std::string of = value;
