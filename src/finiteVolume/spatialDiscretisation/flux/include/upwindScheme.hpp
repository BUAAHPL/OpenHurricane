/*!
 * \file upwindScheme.hpp
 * \brief Headers of upwind scheme.
 *        The subroutines and functions are in the <i>upwindScheme.cpp</i> file.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    ...
    public:
        declareClassNames;
        declareObjFty(upwindScheme, controller,
                      (const controller &cont, const runtimeMesh &mesh, flowModel &flow),
                      (cont, mesh, flow));

        /**\brief Construct null.*/
        inline upwindScheme() = delete;

        upwindScheme(const controller &cont, const runtimeMesh &mesh, flowModel &flow);

        static uniquePtr<upwindScheme> creator(const controller &cont, const runtimeMesh &mesh,
                                               flowModel &flow);

        /**\brief Destructor.*/
        virtual ~upwindScheme() noexcept;
