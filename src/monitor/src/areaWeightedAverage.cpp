/*!
 * \file areaWeightedAverage.cpp
 * \brief Main subroutines for computing the area-weighted average of a quantity through a surface.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    createClassName(areaWeightedAverage);
    registerObjFty(surfaceIntegrals, areaWeightedAverage, controller);
} // namespace OpenHurricane

OpenHurricane::areaWeightedAverage::areaWeightedAverage(const iteration &iter,
...
OpenHurricane::areaWeightedAverage::surIntegral(const cellRealArray &phi) const {
    ...
    return fR / max(tiny, A);
}

hur_nodiscard OpenHurricane::vector
OpenHurricane::areaWeightedAverage::surIntegral(const cellVectorArray &phi) const {
    const auto &faces = mesh().faces();
    const auto &fW = mesh().faceWeight();
    const auto &fA = mesh().faceArea();
    const auto &fzl = mesh().faceZones();
