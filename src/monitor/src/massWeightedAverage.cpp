/*!
 * \file massWeightedAverage.cpp
 * \brief Main subroutines for computing the mass-weighted average of a quantity through a surface.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    createClassName(massWeightedAverage);
    registerObjFty(surfaceIntegrals, massWeightedAverage, controller);
} // namespace OpenHurricane

OpenHurricane::massWeightedAverage::massWeightedAverage(const iteration &iter,
...
OpenHurricane::massWeightedAverage::surIntegral(const cellRealArray &phi) const {
    ...
    return fR / max(tiny, sumA);
}

hur_nodiscard OpenHurricane::vector
OpenHurricane::massWeightedAverage::surIntegral(const cellVectorArray &phi) const {
    const auto &faces = mesh().faces();
    const auto &fW = mesh().faceWeight();
    const auto &fA = mesh().faceArea();
    const auto &fzl = mesh().faceZones();
