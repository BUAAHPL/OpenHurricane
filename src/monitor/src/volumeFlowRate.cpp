/*!
 * \file volumeFlowRate.cpp
 * \brief Main subroutines for computing volume flow rate through a surface.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    createClassName(volumeFlowRate);
    registerObjFty(surfaceIntegrals, volumeFlowRate, controller);
} // namespace OpenHurricane

OpenHurricane::volumeFlowRate::volumeFlowRate(const iteration &iter, const runtimeMesh &mesh,
                                              const controller &cont, const integerList &zd)
    : surfaceIntegrals(iter, mesh, cont, zd) {}

hur_nodiscard OpenHurricane::real
OpenHurricane::volumeFlowRate::surIntegral(const cellRealArray &phi) const {
