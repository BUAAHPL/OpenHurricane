/*!
 * \file chemicallyActicatedBimolecularRR.hpp
 * \brief Main subroutines for chemically acticated bimolecular reaction rate.
 * \author Rao Sihang
 * \version V2.0.0
 * \date 2022.05.02
 *
...
namespace OpenHurricane {
    createClassName(chemicallyActicatedBimolecularRR);
    registerObjFty(reactionRateTypes, chemicallyActicatedBimolecularRR, controller);
} // namespace OpenHurricane

hur_nodiscard OpenHurricane::real
OpenHurricane::chemicallyActicatedBimolecularRR::k(const real p, const real T,
                                               const realArray &c) const {
    real k0 = k0_.k(p, T, c);
    real kinf = kinf_.k(p, T, c);
    real Pr = k0 / notZeroD(kinf) * thirdBodyEfficiency_.M(c);

    return k0 * (1.0 / (1.0 + Pr)) * F_->F(T, Pr);
}

hur_nodiscard OpenHurricane::real
OpenHurricane::chemicallyActicatedBimolecularRR::DkDT(const real kj, const real p, const real T,
                                                  const realArray &c) const {
    const real k0 = k0_.k(p, T, c);
    const real kinf = kinf_.k(p, T, c);
    const real Pr = k0 / notZeroD(kinf) * thirdBodyEfficiency_.M(c);

    return (1.0 / (1.0 + Pr)) * F_->F(T, Pr) * k0_.DkDT(k0, p, T, c);
}

void OpenHurricane::chemicallyActicatedBimolecularRR::gamDGamDCi(const real P, const real T,
                                                             const realArray &c,
...
hur_nodiscard OpenHurricane::real
OpenHurricane::chemicallyActicatedBimolecularRR::gamDGamDT(const real p, const real T,
                                                       const realArray &c) const {
    const real M = thirdBodyEfficiency_.M(c);
...
void OpenHurricane::chemicallyActicatedBimolecularRR::resetThirdBodyEff(
